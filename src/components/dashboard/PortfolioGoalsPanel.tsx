...
export const PortfolioGoalsPanel = ({
  ...
  const [campaigns, setCampaigns] = useState<GoalCampaign[]>([
    {
      ...
      title: '🔥 Sprint Relâmpago de Fim de Mês',
      scope: 'collective',
      targetMetric: 'recovered_amount',
      targetValue: 15000,
      startDate: new Date(Date.now() - 3600000).toISOString().slice(0, 16),
      endDate: new Date(Date.now() + 86400000 * 3).toISOString().slice(0, 16),
      reward: 'Voucher iFood R$ 100 + Folga na Sexta',
      createdAt: new Date().toISOString()
    }
  ]);

  const handleSaveCampaign = (campData: Omit<GoalCampaign, 'id' | 'createdAt'>) => {
    const newCamp: GoalCampaign = {
      id: `camp_${Date.now()}`,
      ...campData,
      createdAt: new Date().toISOString()
    };
  ...
  return (
    <div className="space-y-8 print:p-0 no-print-sections">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        ...
        {portfolioSubTab === 'goals' && (
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            >
              <Trophy size={16} />
              <span>Nova Campanha / PDI</span>
            </button>

            <button 
              onClick={handleExportExcel}
              className="px-4 py-2.5 bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-300 font-bold rounded-xl transition-all flex items-center gap-2 text-xs active:scale-95 shadow-lg cursor-pointer"
            >
      ...
      {/* MODAL UNIFICADO DE CRIAR CAMPANHAS & METAS (PDIs) */}
      <GoalCampaignModal
