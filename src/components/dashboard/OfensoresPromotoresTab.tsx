...
import { UserProfile, Agreement, Team, CustomIndicatorConfig, PerformanceProfileConfig } from '../../types';
import { formatCurrency } from '../../utils/masks';
import { IndicatorConfigModal } from '../modals/IndicatorConfigModal';
...
export const OfensoresPromotoresTab: React.FC<OfensoresPromotoresTabProps> = ({
  ...
  // Configurações Customizadas de Indicadores e Perfis
  const INITIAL_INDICATORS: CustomIndicatorConfig[] = [
    { id: 'ind_conversion', label: 'Meta de Acordo', metricKey: 'conversion' },
    { id: 'ind_revenue', label: 'Faturamento R$', metricKey: 'revenue' },
    { id: 'ind_share', label: 'Share Operação', metricKey: 'share' },
    { id: 'ind_qa', label: 'Nota QA / Script', metricKey: 'qa' },
    { id: 'ind_attendance', label: 'Pontualidade & Presença', metricKey: 'attendance' },
    { id: 'ind_absenteeism', label: 'Taxa de Absenteísmo', metricKey: 'absenteeism' }
  ];

  const INITIAL_PROFILES: PerformanceProfileConfig[] = [
    {
      id: 'balanced',
      title: '⚖️ Equilibrado 360°',
      description: 'Visão balanceada de todos os indicadores.',
      weights: { ind_conversion: 25, ind_revenue: 20, ind_share: 20, ind_qa: 15, ind_attendance: 10, ind_absenteeism: 10 }
    },
    {
      id: 'revenue',
      title: '💰 Foco Faturamento',
      description: 'Prioriza caixa e representatividade financeira.',
      weights: { ind_conversion: 10, ind_revenue: 40, ind_share: 30, ind_qa: 10, ind_attendance: 5, ind_absenteeism: 5 }
    },
    {
      id: 'qa',
      title: '🛡️ Foco QA & Script',
      description: 'Prioriza conformidade e qualidade do atendimento.',
      weights: { ind_conversion: 15, ind_revenue: 15, ind_share: 10, ind_qa: 40, ind_attendance: 10, ind_absenteeism: 10 }
    },
    {
      id: 'attendance',
      title: '⏰ Foco Assiduidade',
      description: 'Prioriza presença e pontualidade da equipe.',
      weights: { ind_conversion: 15, ind_revenue: 15, ind_share: 10, ind_qa: 10, ind_attendance: 25, ind_absenteeism: 25 }
    }
  ];

  const [customIndicators, setCustomIndicators] = useState<CustomIndicatorConfig[]>(() => {
    const stored = localStorage.getItem(`custom_indicators_${profile.organizationId}`);
    return stored ? JSON.parse(stored) : INITIAL_INDICATORS;
  });

  const [customProfiles, setCustomProfiles] = useState<PerformanceProfileConfig[]>(() => {
    const stored = localStorage.getItem(`custom_profiles_${profile.organizationId}`);
  ...
  // Auxiliar para obter a data local YYYY-MM-DD
  ...
  // Filtrar Acordos pelo Período Escolhido com precisão de fuso horário local
  const filteredAgreements = useMemo(() => {
    ...
    return safeAgreements.filter(a => {
      const rawDateStr = a.createdAt || a.dueDate;
      if (!rawDateStr) return true;

      // Trata tanto timestamps ISO quanto strings de data YYYY-MM-DD
      const dateObj = new Date(rawDateStr);
      const itemDateStr = isNaN(dateObj.getTime()) 
        ? rawDateStr.split('T')[0] 
        : getLocalDateString(dateObj);

      if (periodFilter === 'today') return itemDateStr === todayStr;
      if (periodFilter === 'yesterday') return itemDateStr === yesterdayStr;
      if (periodFilter === 'week') return itemDateStr >= startOfWeekStr && itemDateStr <= todayStr;
      if (periodFilter === 'month') return itemDateStr >= startOfMonthStr && itemDateStr <= todayStr;
  ...
  return (
    <div className="space-y-8">
        ...
        <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 pb-4 ${
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-black text-xs transition-all shadow-md active:scale-95 cursor-pointer border-2 border-slate-900 flex items-center gap-1.5"
              title="Adicionar/remover indicadores, editar nomes e ajustar pesos dos perfis"
            >
              <SlidersHorizontal size={15} />
              <span>⚙️ Personalizar Indicadores & Pesos</span>
            </button>
      ...
      {/* Modal de Personalização de Indicadores & Pesos */}
      <IndicatorConfigModal
        isOpen={isConfigModalOpen}
        onClose={() => setIsConfigModalOpen(false)}
        indicators={customIndicators}
        onSaveIndicators={(newInds, weightsMap) => {
          setCustomIndicators(newInds);
          localStorage.setItem(`custom_indicators_${profile.organizationId}`, JSON.stringify(newInds));
        }}
        profiles={customProfiles}
        activePresetId={activePreset}
        onSaveProfile={(profId, updatedWeights) => {
          const updated = customProfiles.map(p => p.id === profId ? { ...p, weights: updatedWeights } : p);
          setCustomProfiles(updated);
          localStorage.setItem(`custom_profiles_${profile.organizationId}`, JSON.stringify(updated));

          if (profId === activePreset) {
            if (updatedWeights.ind_conversion !== undefined) setWeightConversion(updatedWeights.ind_conversion);
            if (updatedWeights.ind_revenue !== undefined) setWeightRevenue(updatedWeights.ind_revenue);
            if (updatedWeights.ind_share !== undefined) setWeightShare(updatedWeights.ind_share);
            if (updatedWeights.ind_qa !== undefined) setWeightQa(updatedWeights.ind_qa);
            if (updatedWeights.ind_attendance !== undefined) setWeightAttendance(updatedWeights.ind_attendance);
            if (updatedWeights.ind_absenteeism !== undefined) setWeightAbsenteeism(updatedWeights.ind_absenteeism);
          }
        }}
        theme={theme}
        showToast={showToast}
