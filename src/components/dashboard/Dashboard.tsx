...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  // Modo Comparação de Dados
  const [isComparisonMode, setIsComparisonMode] = useState(false);
  const [comparisonType, setComparisonType] = useState<'prev' | 'yoy'>('prev');
  ...
  // Calcula o intervalo do período de comparação baseado no cardDateFilter e comparisonType
  const comparisonAgreements = useMemo(() => {
    ...
    if (cardDateFilter === 'today') {
      ...
      if (comparisonType === 'prev') {
        return memberFilteredAgreements.filter(a => {
          const d = new Date(a.createdAt);
          return d >= yesterday && d < dayBefore;
        });
      } else {
        // YoY: mesmo dia do ano passado
        const sameLastYear = new Date(today);
        sameLastYear.setFullYear(today.getFullYear() - 1);
        const sameLastYearEnd = new Date(sameLastYear);
        sameLastYearEnd.setDate(sameLastYear.getDate() + 1);
        return memberFilteredAgreements.filter(a => {
          const d = new Date(a.createdAt);
          return d >= sameLastYear && d < sameLastYearEnd;
        });
      }
    } else if (cardDateFilter === 'week') {
      ...
      startOfThisWeek.setDate(today.getDate() - distToMon);
      if (comparisonType === 'prev') {
        // Semana anterior
        ...
        // YoY: mesma semana do ano passado
        const startOfLastYearWeek = new Date(startOfThisWeek);
        startOfLastYearWeek.setFullYear(startOfThisWeek.getFullYear() - 1);
        const endOfLastYearWeek = new Date(startOfLastYearWeek);
        endOfLastYearWeek.setDate(startOfLastYearWeek.getDate() + 7);
        return memberFilteredAgreements.filter(a => {
          const d = new Date(a.createdAt);
          return d >= startOfLastYearWeek && d < endOfLastYearWeek;
        });
      }
    } else if (cardDateFilter === 'month') {
      if (comparisonType === 'prev') {
        // Mês anterior
        const compMonth = selectedMonth === 0 ? 11 : selectedMonth - 1;
        const compYear = selectedMonth === 0 ? selectedYear - 1 : selectedYear;
        return memberFilteredAgreements.filter(a => {
          const d = new Date(a.createdAt);
          return d.getMonth() === compMonth && d.getFullYear() === compYear;
        });
      } else {
      ...
      if (comparisonType === 'prev') {
        ...
        return memberFilteredAgreements.filter(a => {
          const d = new Date(a.createdAt);
          return d.getFullYear() === selectedYear - 2;
        });
      }
    }
    return [];
  }, [isComparisonMode, comparisonType, cardDateFilter, memberFilteredAgreements, selectedMonth, selectedYear]);
  ...
  const comparisonStats = useDashboardStats({
    monthAgreements: memberFilteredAgreements,
    filteredAgreements: isComparisonMode && comparisonAgreements.length > 0 ? comparisonAgreements : cardTimeFilteredAgreements,
    monthlyGoal,
    selectedMonth,
    selectedYear
  });
          ...
          <main className="max-w-7xl w-full mx-auto px-6 py-8 space-y-8 no-print">
            ...
            {dashboardTab === 'financial' && (
              <div className={`flex flex-col gap-3 p-4 rounded-2xl border transition-all ${
                ...
                {/* Sub-linha para Datas Customizadas ou Opções do Modo Comparação */}
                {(cardDateFilter === 'custom' || isComparisonMode) && (
                    ...
                    {isComparisonMode && (
                      <>
                        <div className={`flex items-center p-1 rounded-xl border ${
                          ...
                          <button
                            type="button"
                            onClick={() => setComparisonType('prev')}
                        ...
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                          theme === 'dark' ? 'text-violet-300 bg-violet-500/10 border-violet-500/20' : 'text-violet-700 bg-violet-50 border-violet-200'
                        }`}>
                          {comparisonType === 'prev'
                            ? cardDateFilter === 'today' ? 'vs. Ontem'
                              : cardDateFilter === 'week' ? 'vs. Semana Passada'
                              : cardDateFilter === 'month' ? 'vs. Mês Anterior'
                              : cardDateFilter === 'year' ? 'vs. Ano Anterior'
                              : 'vs. Período Anterior'
                            : cardDateFilter === 'year' ? 'vs. Dois Anos Atrás'
            ...
            >
              ...
              {dashboardTab === 'financial' && (
                <div className="space-y-8">
                  {/* Navegação de Sub-abas do Módulo Acordos & Operação (visível apenas para Operadores, Supervisores e Admins) */}
                  {profile.role !== 'manager' && profile.role !== 'coordinator' && (
                    <div className={`p-1.5 rounded-2xl border flex items-center gap-1.5 overflow-x-auto ${
                      ...
                      >
                        <span>💰 Painel Financeiro & Operação</span>
                      </button>
                      <button
                        onClick={() => setFinancialSubTab('charge')}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 whitespace-nowrap ${
                          financialSubTab === 'charge'
                            ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20'
                            : theme === 'dark' ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-600 hover:bg-slate-50'
                        }`}
                      >
                        <span>📥 Carga & Importação de Acordos</span>
                      </button>
                    </div>
                  )}

                  {financialSubTab === 'charge' && profile.role !== 'manager' && profile.role !== 'coordinator' ? (
                    <BackOfficeTab 
                      profile={profile}
                      showToast={showToast}
                      theme={theme}
                  ...
                  <StatsGrid 
                    stats={stats}
                    comparisonStats={isComparisonMode ? comparisonStats : undefined}
              ...
              {(dashboardTab === 'bi' || dashboardTab === 'portfolio') && (
                <div className="space-y-6">
                  {/* Navegação de Sub-abas (Visível apenas com múltiplas opções para cargos de gestão/supervisão) */}
                  {profile.role !== 'member' && (
                    <div className={`p-1.5 rounded-2xl border flex items-center gap-1.5 overflow-x-auto ${
                      theme === 'dark' ? 'bg-slate-900/40 border-white/5' : 'bg-white border-slate-200 shadow-sm'
                    }`}>
                      <button
                        onClick={() => setBiMasterSubTab('analytics')}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 whitespace-nowrap ${
                          biMasterSubTab === 'analytics'
                            ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20'
                            : theme === 'dark' ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-600 hover:bg-slate-50'
                        }`}
                      >
                        <span>🗺️ BI Estratégico & Mapa de Calor</span>
                      </button>

                      <button
                        onClick={() => setBiMasterSubTab('goals')}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-2 whitespace-nowrap ${
                          biMasterSubTab === 'goals'
                            ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20'
                            : theme === 'dark' ? 'text-slate-400 hover:text-white hover:bg-white/5' : 'text-slate-600 hover:bg-slate-50'
                        }`}
                      >
                        <span>🎯 Cockpit de Metas, PDIs & Ofensores</span>
                      </button>
                    </div>
                  )}
              ...
              {/* CONTEÚDO DA ABA DE GESTÃO DE PESSOAS & COORDENAÇÃO GERAL */}
              {(dashboardTab === 'coordination' || dashboardTab === 'people') && (
                <div className="space-y-8">
                  {/* Cabeçalho */}
        ...
        <div className="border-b-4 border-slate-900 pb-6 mb-8 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm font-bold text-slate-700">
              {organizationName || 'Traker'} {organizationCnpj && `| CNPJ: ${organizationCnpj}`}
            </p>
          </div>
