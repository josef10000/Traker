...
export const QaDashboard = ({
  ...
  const handleSaveEvaluation = async (data: {
    ...
    if (profile.organizationId === 'sandbox-test') {
      ...
      sandboxService.setQaEvaluation({
        ...
        callLink: data.callLink || undefined,
        callExpiresAt: data.callExpiresAt || undefined,
        grades: data.grades,
        feedback: data.feedback,
        createdAt: now,
        recoveredAmount: data.recoveredAmount,
        delayProfileLabel: data.delayProfileLabel || undefined,
    ...
    try {
      ...
      await setDoc(doc(db, 'qa_evaluations', evalId), {
        ...
        delayProfileLabel: data.delayProfileLabel || null,
  ...
  const handleToggleBestPractice = async (evalId: string, currentVal: boolean) => {
    const newVal = !currentVal;
    if (profile.organizationId === 'sandbox-test') {
      const updated = evaluations.map(e => e.id === evalId ? { ...e, isBestPractice: newVal } : e);
      setEvaluations(updated);
      showToast(newVal ? 'Marcado como Áudio Destaque (Best Practice)!' : 'Destaque removido.', 'success');
      return;
    }

    try {
      await updateDoc(doc(db, 'qa_evaluations', evalId), {
        isBestPractice: newVal
      });
      showToast(newVal ? 'Marcado como Áudio Destaque (Best Practice)!' : 'Destaque removido.', 'success');
    } catch (err) {
      console.error(err);
