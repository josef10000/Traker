...
export const DailyAgendaSection = ({
  ...
  const [isCollapsed, setIsCollapsed] = React.useState<boolean>(() => {
    try {
      return localStorage.getItem('tracker_agenda_collapsed') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const toggleCollapse = () => {
    setIsCollapsed(prev => {
      const next = !prev;
      try {
        localStorage.setItem('tracker_agenda_collapsed', String(next));
      } catch (e) {
        console.error(e);
      }
      return next;
  ...
  return (
    <section className={`glass-card p-6 rounded-[2rem] border space-y-4 transition-all ${
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer active:scale-95 ${
              theme === 'dark'
                ? 'bg-slate-800/60 hover:bg-slate-700 text-slate-300 hover:text-white border-slate-700/50'
                : 'bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 border-slate-200'
            }`}
            title={isCollapsed ? "Expandir Agenda do Dia" : "Recolher Agenda do Dia"}
          >
