...
| **🚀 Onboarding Corporativo (/accept-invite)** | Rota dedicada de ativação de convite com e-mail corporativo bloqueado (*read-only*), vinculação atômica ao Firestore/Auth e disparo de e-mails via Vercel Serverless Function (Resend API) com suporte a compartilhamento em 1-clique no WhatsApp. |
...
## 🛡️ CI/CD & Segurança End-to-End

O repositório adota padrões de **Hardening de Segurança e Conformidade Corporativa**:
...
- **SonarCloud Analysis**: Análise contínua com Rating A em Segurança, Confiabilidade e Manutenibilidade.
- **Isolamento de Endpoints Serverless**: Whitelist estrita de CORS (`system-status.ts` e `send-email.ts`) e proteção contra timing attacks via `crypto.timingSafeEqual()` no Webhook CRM.
- **Integridade Criptográfica (SHA-256)**: Verificação simétrica da cadeia de auditoria encadeada (`auditIntegrity.ts`), assegurando conformidade estrita com a LGPD.
- **2FA com Entropia Criptográfica**: Backup codes e biometria FIDO2 gerados com `crypto.getRandomValues()`.
- **Tags de Restauração**: Restauração garantida via tag de versão (`backup-pre-security-fixes-20260814`).
