...
export const AttendanceCalendarSection: React.FC<AttendanceCalendarSectionProps> = ({
  ...
  useEffect(() => {
    getNationalHolidays(year).then(setHolidays);
  }, [year]);
  ...
  const daysArray = Array.from({ length: totalDays }, (_, i) => {
    ...
    const holiday = isNationalHoliday(dateStr, holidays);

    return { dayNum, dayOfWeek, isWeekend, dateStr, holiday };
  });
    ...
    <div className={`border rounded-3xl p-6 space-y-6 shadow-xl ${
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className={`flex items-center gap-3 p-1.5 rounded-2xl border ${
          ...
          <span className="text-xs font-black capitalize min-w-[130px] text-center tracking-wide">
            {monthName}
          </span>
          ...
          <thead>
            <tr className={theme === 'dark' ? 'bg-slate-950/40' : 'bg-slate-50'}>
              ...
              {daysArray.map(day => (
                <th
                  key={day.dayNum}
                  className={`p-2 text-center text-[10px] font-black border-r border-white/5 min-w-[42px] ${
                    day.holiday
                  ...
                  title={day.holiday ? `🎉 Feriado Nacional: ${day.holiday.name}` : undefined}
                >
                  <span className="block leading-none">{day.dayNum}</span>
                  <span className="block text-[8px] mt-1 font-semibold uppercase opacity-80 leading-none">
                    {day.holiday ? '🇧🇷' : day.dayOfWeek}
                  </span>
                </th>
              ))}
            </tr>
