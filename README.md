...
* **⚙️ Central de Configurações Universal & Governança de Canais**: Modal corporativo de configurações (`SettingsModal.tsx`) integrado ao cabeçalho. Para a Liderança (Supervisores, Gerentes, Coordenadores e Super Admins), libera a conciliação de acordos, o cadastro e gerenciamento de canais de atendimento (`VOZ`, `CHAT`, `WEBPHONE`, `OKTOR`, `SALESFORCE`, `WHATSAPP`, `QUITE DIGITAL`, etc.) e do catálogo de motivos de tabulação da empresa. Operadores e Supervisores têm acesso exclusivo à criação de novos acordos. Operadores, QA e Backoffice utilizam o modal para atalhos e preferências pessoais.
