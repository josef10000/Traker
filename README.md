...
* **📊 BI & Analytics Estratégico & Módulo de Tabulação**: A aba **BI & Analytics Estratégico** (`BiAnalyticsTab.tsx`) centraliza exclusivamente o **Mapa de Calor Multidimensional Reativo (Seg-Sáb x 08h-20h)**, as **3 Métricas Avançadas (Ticket Médio R$, Resolução 1º Contato FCR % e Taxa de Quebra Futura %)**, o **Pareto Rosca de Motivos** e os **Insights Preditivos**. A aba **Tabulação** (`AttendanceTabulationTab.tsx`) permanece focada no histórico operacional enxuto com a governança pelo Supervisor, KPIs originais (Conversão Real %, Negociações Reais, Acordos Firmados, Total Atendimentos) e paginação de 8 itens.
...
* **⏱️ Aba Jornada & Cockpit de Atividade Hora a Hora (Banco de Pausas 72m)**: Nova aba dedicada na Sidebar para supervisão e gestão. Monitora reativamente a produção hora a hora (08h às 19h) compilando passivamente acordos, atendimentos e consultas no sistema. Oferece tolerância inteligente por **Banco Geral de Pausas Cumulativas de 72 minutos por dia** (com fracionamento livre), calibração por turno individual e restrição estrita de escopo por hierarquia de cargo (Supervisores enxergam apenas suas equipes; Coordenadores/Gerentes enxergam suas carteiras/visão global).
