...
* **📡 Resiliência Offline-First & Sincronização Automática**: Para operadores logados com instabilidade ou falta de sinal de internet, o sistema permite fechar acordos, agendar retornos e registrar tabulações normalmente. Todos os dados são salvos localmente em `IndexedDB` com resposta visual imediata (*Optimistic UI*) e sincronizados automaticamente na nuvem assim que a internet reconectar. O cabeçalho exibe um badge visual `📡 Modo Offline (Gravando Localmente)` quando desconectado e uma notificação de sucesso ao reconectar.
